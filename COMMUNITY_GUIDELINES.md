# 🤝 Community Guidelines - CODERCOPS Discord Server

**Welcome to the CODERCOPS community for 100 Days of AI Engineer!**

**These guidelines ensure we all have a productive, supportive, and respectful learning environment.**

---

## 🎯 Our Mission

**Build AI Engineers through:**

- Structured learning
- Public accountability
- Peer support
- Knowledge sharing
- Professional networking

**We're here to help each other succeed.**

---

## ✅ Core Values

### **1. Learning in Public**
Share your progress, wins, failures, and questions openly.

### **2. Help First**
If you know the answer, share it. We rise together.

### **3. No Gatekeeping**
No "stupid questions." Everyone was a beginner once.

### **4. Quality Over Speed**
Finished 100 days with deep learning > rushed through checking boxes.

### **5. Respectful Discourse**
Disagree on ideas, not people. Stay professional.

---

## 📋 Server Rules

### **Rule 1: Be Respectful**

**DO:**

✅ Treat everyone with respect
✅ Give constructive feedback
✅ Assume good intentions
✅ Be patient with beginners

**DON'T:**

❌ Insult, mock, or belittle anyone
❌ Use discriminatory language
❌ Attack people for not knowing something
❌ Be condescending ("obviously," "just," "simply")

**Violation:** Warning → Timeout → Ban

---

### **Rule 2: Stay On Topic**

**#100daysofaiengineer channel:**

- Daily check-ins
- Progress updates
- Quick questions
- Wins and struggles
- Accountability posts

**100daysofaiengineer forum:**

- Weekly reviews
- Project showcases
- Code reviews
- Detailed questions
- Long-form discussions

**#general or other channels:**

- Off-topic chat
- AI news
- Career discussions
- Networking

**Keep channels organized. Don't spam.**

---

### **Rule 3: No Spam or Self-Promotion Without Value**

**Allowed:**

✅ Share your projects (with context)
✅ Share helpful resources
✅ Post progress updates
✅ Share job opportunities (relevant to AI/ML)
✅ Blog posts (if educational and not excessive)

**Not Allowed:**

❌ Unsolicited DMs promoting products/services
❌ Affiliate links without disclosure
❌ Course promotions (unless exceptionally relevant and disclosed)
❌ Crypto/NFT/get-rich-quick schemes
❌ Repeated self-promotion without contributing value

**Violation:** Warning → Removal of content → Ban

---

### **Rule 4: Post in the Right Place**

**Use the right channel/forum:**

| Content | Where to Post |
|---------|---------------|
| Daily progress update | #100daysofaiengineer channel |
| Weekly review | 100daysofaiengineer forum |
| Code review request | 100daysofaiengineer forum |
| Quick question | #100daysofaiengineer channel |
| Detailed technical question | 100daysofaiengineer forum |
| Job posting | #jobs (if exists) or relevant channel |
| Off-topic | #general |
| Introductions | #introductions (if exists) |

**Don't cross-post the same content to multiple channels.**

---

### **Rule 5: Use Threads and Formatting**

**For longer discussions:**

- Use forum posts (not channel messages)
- Use threads to keep conversations organized
- Use code blocks for code:

  ```python

  # Like this

  ```

- Use markdown for formatting

**Makes it easier for everyone to follow.**

---

### **Rule 6: No Plagiarism**

**DO:**

✅ Share code you wrote
✅ Credit sources when you adapt code
✅ Explain how you modified tutorial code
✅ Link to resources you learned from

**DON'T:**

❌ Copy-paste someone else's project and claim it's yours
❌ Submit tutorial code as your own work
❌ Plagiarize blog posts or documentation

**We're here to learn. Honesty is required.**

**Violation:** Removal from challenge + potential ban

---

### **Rule 7: Help Others**

**If you can answer a question, do it.**

**Expected behavior:**

- Answer questions when you know the answer
- Review code when requested (if you have time)
- Encourage people who are struggling
- Share resources that helped you
- Celebrate others' wins

**Community karma: What goes around comes around.**

---

### **Rule 8: No Ghosting**

**If you miss days or need to pause:**

- Post about it in Discord
- Explain what happened
- Ask for support
- Don't just disappear

**Related:** [FAILURE_RECOVERY.md](FAILURE_RECOVERY.md)

**We can't help if you ghost. Stay engaged even when struggling.**

---

### **Rule 9: Protect Privacy & Security**

**DO:**

✅ Share code on GitHub (public repos)
✅ Share learning progress
✅ Network professionally

**DON'T:**

❌ Share API keys, passwords, credentials
❌ Share personal information of others
❌ Dox or harass anyone
❌ Share confidential company information

**Keep it professional and secure.**

---

### **Rule 10: Follow Discord ToS**

**All members must follow Discord's Terms of Service and Community Guidelines.**

This includes but not limited to:

- No illegal content
- No harassment
- No hate speech
- No explicit content
- Must be 13+ (per Discord ToS)

**Link:** https://discord.com/terms

---

## 📱 Daily Check-In Requirements

### **What to Post Daily in #100daysofaiengineer:**

**Minimum (5 min):**

```

Day X/100 ✅

Topic: [What you learned]
Time: [Hours spent]
Done: [What you completed]
Code: [GitHub link]

#100DaysOfAIEngineer

```

**Better (recommended):**

```

Day X/100 ✅

🎯 Today's Focus: [Topic]
⏰ Time: [X] hours

✅ Completed:
  - [Task 1]
  - [Task 2]
  - [Task 3]

💡 Key Learning: [Your biggest insight]

🔗 Code: [GitHub link]

💪 Tomorrow: [What's next]

#100DaysOfAIEngineer

```

**Best (when you have time):**

```

Day X/100 ✅

🎯 Today's Focus: [Topic + context]

✅ What I Built:

[Brief description of project/code]

💡 Key Insights:

1. [Learning 1]
2. [Learning 2]

🤔 Challenges:
[What was hard + how you solved it]

❓ Question:
[Something you're still unsure about]

🔗 Links:
Code: [GitHub]
Demo: [Link if applicable]

📊 Streak: [X] days
⏰ Time: [X] hours

Tomorrow: [Next topic]

#100DaysOfAIEngineer #Day[X]

```

---

## 🎯 Weekly Forum Post Requirements

**Every 7 days, post detailed review in forum:**

**Template:** See [weekly_reviews/](weekly_reviews/) for specific week templates

**Minimum requirements:**

- Summary of week's learning
- Projects built
- Challenges overcome
- Code links
- Request for peer review

**Related:** [PEER_REVIEW_GUIDE.md](PEER_REVIEW_GUIDE.md)

---

## 👥 How to Ask for Help

### **Good Question Format:**

```

**Problem:** [Clear, specific description]

**Day/Topic:** Day X - [Topic]

**What I'm trying to do:**
[Your goal]

**What's happening:**
[Current behavior]

**Error message:**

```

[Paste full error]

```

**What I've tried:**

1. [Attempt 1]
2. [Attempt 2]
3. [Researched X, Y, Z]

**Code:**
[GitHub link or code snippet]

**Environment:**

- Python version:
- Libraries:
- OS:

Any ideas? 🙏

```

**This makes it easy for others to help you.**

---

### **How NOT to Ask:**

❌ "My code doesn't work, help!"
❌ "Can someone fix this for me?"
❌ "Why isn't this working?" [no context]
❌ Screenshot of code (use code blocks or GitHub)
❌ Asking people to DM you (ask publicly)

**Public questions = everyone learns.**

---

## 🤝 How to Give Help

### **Good Answer Format:**

```

**Short answer:** [Quick solution if simple]

**Explanation:** [Why this works]

**Code example:**

```python

# Your suggested code

```

**Resources:**

- [Link to docs]
- [Link to tutorial]

**Next steps:**
[What they should try next]

```

**Don't just give the answer. Help them understand.**

---

### **When You Don't Know:**

**It's OK to say:**

- "I haven't learned this yet, but [tag someone who has]"
- "Not sure, but have you tried [resource]?"
- "I'm learning this too, let's figure it out together"

**You don't have to know everything. Just be helpful.**

---

## 🚫 What Gets You Banned

**Immediate Ban (no warning):**

- Harassment, doxxing, threats
- Hate speech, discrimination
- Spam bots
- Scams or phishing
- Sharing illegal content
- Violating Discord ToS

**Ban After Warnings:**

- Repeated violations of community rules
- Consistent disrespectful behavior
- Ignoring mod requests
- Plagiarism / cheating

**We want everyone to succeed, but not at others' expense.**

---

## 👮 Moderator Actions

**Moderators can:**

- Issue warnings
- Timeout users (temporary mute)
- Delete messages
- Ban users
- Make final decisions on edge cases

**If you disagree with mod action:**

- DM mods respectfully
- Explain your perspective
- Accept final decision

**Mods are volunteers keeping community healthy. Respect them.**

---

## 🎉 Community Events

### **Monthly Celebrations:**

- **First Friday:** New cohort kickoff
- **Last Friday:** Monthly graduation ceremony
- **Weekly:** Code review sessions

**Participate when you can. Build connections.**

---

## 💬 Communication Best Practices

### **When Posting:**

**Be Clear:**

- Specific subject lines
- Context for questions
- Links to relevant info

**Be Concise:**

- TL;DR for long posts
- Use bullet points
- Break up walls of text

**Be Helpful:**

- Add value to conversations
- Share what worked for you
- Encourage others

### **When Responding:**

**Be Kind:**

- Remember you were a beginner
- No condescending tone
- Assume good faith

**Be Constructive:**

- Critique ideas, not people
- Offer alternatives
- Explain reasoning

**Be Timely:**

- Respond when you can
- But don't feel obligated 24/7
- Upvote good questions if you can't answer

---

## 📊 Accountability & Streaks

**Discord shows your consistency:**

- Daily posts visible to all
- Community calls out if you go silent
- Accountability partners check on you

**This is a feature, not a bug.**

**We keep each other accountable. That's why it works.**

---

## 🌟 Recognition System

**Community recognizes:**

- 🏆 Graduates (completed 100 days)
- 🔥 Perfect streaks
- 🤝 Most helpful members
- 💪 Comeback completers
- 🚀 Got hired

**Contribute to community, get recognized.**

---

## 🔗 External Communication

**Representing CODERCOPS / 100 Days Challenge elsewhere:**

**When posting on LinkedIn, Twitter, etc.:**

✅ Tag @CODERCOPS
✅ Use #100DaysOfAIEngineer
✅ Link back to Discord
✅ Share wins and lessons

**We grow together. Help others find the community.**

---

## 📝 Reporting Issues

**If you see rule violations:**

1. **Don't engage** - Don't argue with rule-breakers
2. **Flag the message** - Use Discord's report feature
3. **Tag mods** - @moderator in the message
4. **DM mods** - If it's sensitive

**Mods will handle it.**

---

## 🎯 Bottom Line

**This community exists to help YOU succeed.**

**Rules exist to keep it productive and respectful.**

**Follow the guidelines. Help others. Learn in public.**

**Together, we become AI Engineers.** 💪🔥

---

## 🔗 Related Docs

- 👥 [COMMUNITY.md](COMMUNITY.md) - How to join and engage
- 📊 [ACCOUNTABILITY.md](ACCOUNTABILITY.md) - Daily requirements
- 👀 [PEER_REVIEW_GUIDE.md](PEER_REVIEW_GUIDE.md) - Code review process
- 🏆 [HALL_OF_FAME.md](HALL_OF_FAME.md) - Graduates recognition

---

**Welcome to CODERCOPS. Let's build AI Engineers.** 🚀

**Discord:** https://discord.gg/9eFXYntYa8
**All Socials:** @CODERCOPS
